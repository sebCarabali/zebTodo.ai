...
- **Decisión final sobre AC #4 (2026-06-22):** el usuario confirmó que **aún no tiene cuenta de Azure**. No es una tarea pendiente de ejecutar, es un prerrequisito (cuenta + suscripción de Azure) que todavía no existe — por lo tanto Subtask 5.1 no puede completarse en este momento bajo ningún escenario, y por extensión AC #4 (despliegue automático real a Azure Container Apps) no puede verificarse end-to-end todavía. Por decisión explícita del usuario, esto **no bloquea el cierre de esta historia**: se marca Status = `review` con Tasks 1-4 completas y verificadas, y AC #4 queda como **seguimiento pendiente separado** para cuando exista una cuenta de Azure. En ese momento, retomar exactamente desde Subtask 5.1: `azd auth login` → `azd pipeline config` (ejecutado desde la rama `dev`, donde vive `azure.yaml`) → push a `main` para validar `azd deploy` end-to-end.
...
| Fecha | Cambio |
|---|---|
| 2026-06-22 | Implementación de las Tasks 1-4 completa (solución .NET Aspire Starter inicializada, proyecto Mobile (MAUI) agregado, arranque local orquestado verificado, proyecto xUnit + pipeline CI/CD `build-and-test` creado y verificado en verde). Task 5 (deploy) armada y condicionada a `main`, pero **no se puede completar**: el usuario confirmó que aún no tiene cuenta de Azure, por lo que Subtask 5.1 (`azd pipeline config`) y AC #4 quedan como seguimiento pendiente separado, sin bloquear el cierre de la historia (decisión explícita del usuario). Se descubrió y reconcilió historia de git divergente en `origin/main` (proyecto Todo API preexistente no relacionado); por decisión del usuario se creó la estrategia de ramas `dev` → `sit` → `main`, y el trabajo de esta historia vive en `dev` en espera de promoción. Status → `review`. |
